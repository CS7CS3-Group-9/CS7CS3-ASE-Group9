...
def test_manage_users_post_saves_hashed_user_to_file(tmp_path):
    ...
    assert saved["bob"]["role"] == "user"
    assert "password_hash" in saved["bob"]
    assert "password" not in saved["bob"]
    assert check_password_hash(saved["bob"]["password_hash"], "s3cret-pass")
