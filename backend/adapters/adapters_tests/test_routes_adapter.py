...
def mock_api_response():
    ...
    return {
        ...
        "duration": "278s",
        "encoded_polyline": "_jmeIhgpd@CmBGa@qAqBo@q@y@a@gBm@c@KoEk@_@WWg@Qs@g@cEw@gHOgBq@wFOq@aC}G}@uBiEyHaDcF_@iAYaBOyAO{CAyBFwHNsB?eA}@qPAeAHsAh@uAsAcEeAuHM}EWuLiAm_@AyAFgAn@gEj@qCNyAB{B"
    }
...
def test_fetch_success(mock_post, adapter, mock_api_response):
    ...
    assert recommendation.route == "_jmeIhgpd@CmBGa@qAqBo@q@y@a@gBm@c@KoEk@_@WWg@Qs@g@cEw@gHOgBq@wFOq@aC}G}@uBiEyHaDcF_@iAYaBOyAO{CAyBFwHNsB?eA}@qPAeAHsAh@uAsAcEeAuHM}EWuLiAm_@AyAFgAn@gEj@qCNyAB{B"
...
def test_fetch_missing_polyline(mock_post, adapter):
    ...
    mock_response.json.return_value = {
        ...
        "destination": {"input": "portrane", "coordinates": {"lat": 53.49, "lng": -6.11}},
        "distance_meters": 2823,
        "duration": "278s"
...
def test_multistop_route_order(mock_post, adapter, mock_multistop_response):
    ...
    assert recommendation.multi_stop_data.optimal_route_order == ["swords", "donabate", "portrane"]
...
def test_multistop_with_multiple_waypoints(mock_post, adapter):
    ...
    mock_response.json.return_value = {
        ...
        "number_of_stops": 4,
        "legs": [
            {"from": "A", "to": "B", "duration_seconds": 600, "duration_formatted": "10m", "distance_meters": 5000, "distance_km": 5.0},
            {"from": "B", "to": "C", "duration_seconds": 700, "duration_formatted": "11m", "distance_meters": 5500, "distance_km": 5.5},
            {"from": "C", "to": "D", "duration_seconds": 700, "duration_formatted": "11m", "distance_meters": 5000, "distance_km": 5.0}
        ]
    }
    mock_response.raise_for_status = Mock()
    mock_post.return_value = mock_response
