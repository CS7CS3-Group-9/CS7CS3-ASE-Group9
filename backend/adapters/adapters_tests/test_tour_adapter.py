...
def test_source_name(adapter):
    """Test source name returns 'tour'"""
    assert adapter.source_name() == "tour"
...
def test_fetch_success(mock_post, adapter, mock_overpass_response):
    ...
    assert snapshot.search_radius_km == 5
    assert isinstance(snapshot.timestamp, datetime)
    assert snapshot.source_status == {"overpass": "live"}
...
def test_attraction_parsing(mock_post, adapter, mock_overpass_response):
    ...
    snapshot = adapter.fetch(location="donabate", radius_km=5)
    attractions = snapshot.metrics.attractions
...
def test_attractions_by_type(mock_post, adapter, mock_overpass_response):
    ...
    snapshot = adapter.fetch(location="donabate", radius_km=5)
    by_type = snapshot.metrics.attractions_by_type
...
def test_fetch_empty_response(mock_post, adapter):
    ...
    assert snapshot.metrics.total_attractions == 0
    assert len(snapshot.metrics.attractions) == 0
    assert snapshot.metrics.free_attractions_count == 0
    assert snapshot.metrics.paid_attractions_count == 0
...
def test_attraction_without_optional_fields(mock_post, adapter):
    ...
    attraction = snapshot.metrics.attractions[0]
...
def test_wheelchair_accessibility_variations(mock_post, adapter):
    ...
    assert snapshot.metrics.wheelchair_accessible_count == 1
