...
def mock_api_response():
    ...
    polyline = (
        "_jmeIhgpd@CmBGa@qAqBo@q@y@a@gBm@c@KoEk@_@WWg@Qs@g@cEw@gHOgBq@wFOq@aC}G}@uBiEyHaDcF_"
        "@iAYaBOyAO{CAyBFwHNsB?eA}@qPAeAHsAh@uAsAcEeAuHM}EWuLiAm_@AyAFgAn@gEj@qCNyAB{B"
    )
    return {
        "origin": {"input": "donabate", "coordinates": {"lat": 53.4853365, "lng": -6.1504482}},
        "destination": {"input": "portrane", "coordinates": {"lat": 53.49319999999999, "lng": -6.11462}},
...
def test_fetch_success(mock_post, adapter, mock_api_response):
    ...
    assert recommendation.route == expected_polyline
...
def test_fetch_both_times_specified(mock_post, adapter):
    ...
    with pytest.raises(ValueError):
        adapter.fetch(start="donabate", end="portrane", dep_time=datetime.now(), arr_time=datetime.now())
