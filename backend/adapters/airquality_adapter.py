...
from models.airquality_models import PollutantLevels, AirQualityMetrics, AirQualitySnapshot
...
class AirQualityAdapter(DataAdapter):
    ...
    def fetch(self, latitude: float, longitude: float) -> AirQualitySnapshot:
        ...
        # Parse pollutants
        pollutants = self._parse_pollutants(data["current"], data.get("current_units", {}))
    ...
    def _calculate_metrics(self, aqi_value: int, pollutants: PollutantLevels) -> AirQualityMetrics:
        ...
        return AirQualityMetrics(aqi_value=aqi_value, pollutants=pollutants)
