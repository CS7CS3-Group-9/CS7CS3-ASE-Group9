...
@patch("backend.api.endpoints.health._check_adapter_status", side_effect=_mock_all_configured)
...
def test_health_lists_known_adapters(mock_check, client):
    ...
    for adapter_name in expected_adapters:
        assert adapter_name in adapters, f"Expected adapter '{adapter_name}' in health response"
...
def test_health_adapter_status_values(mock_check, client):
    ...
    for name, status in adapters.items():
        assert isinstance(status, str), f"Adapter '{name}' status should be a string, got {type(status)}"
        assert status in ("configured", "unavailable"), f"Adapter '{name}' has unexpected status: {status}"
...
def test_health_adapter_unavailable_when_import_fails(client):
    ...
    with patch("backend.api.endpoints.health._check_adapter_status", side_effect=_mock_one_fails):
        response = client.get("/health")
        data = response.get_json()
