...
def build_adapter_specs(include: list[str], radius_km: float) -> list[AdapterCallSpec]:
    specs: list[AdapterCallSpec] = []

    if "bikes" in include:
        specs.append(AdapterCallSpec(adapter=BikesAdapter(), kwargs={"radius_km"}))
    ...
    if "airquality" in include:
        specs.append(AdapterCallSpec(adapter=AirQualityAdapter(), kwargs={}))
...
def get_snapshot():
    ...
    adapter_specs = build_adapter_specs(include, radius_km)
